...
- **Repomix export script** (`ai/scripts/generate-repomix-exports.ts`) — Node script that reads `ai/repomix-state.json`, increments the version, runs `npx repomix` to generate two repo packs (full and core), gathers git history (last 10 commits, files changed), and writes three files under `ai/exports/`:
