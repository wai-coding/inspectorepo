...
- **Repomix export script** (`ai/scripts/generate-repomix-exports.ts`) — Node script that reads `ai/repomix-state.json`, increments the version, runs `npx repomix` to generate a full repo pack, gathers git history (last 10 commits, files changed), and writes two files under `ai/exports/`:
  - `repo-pack-vN.md` — full repository pack generated by repomix
  - `changes-summary-vN.md` — milestone header, latest merge info, commit log, changed files, regeneration instructions
- **Version tracker** (`ai/repomix-state.json`) — tracked JSON file with `currentVersion` counter (baseline 4). Incremented by the script each run.
...
- **Prompt master update** — added "Post-Merge Repomix Export" section documenting the workflow: sync main, run repopack, verify files, stop, instruct user to upload.
...
npm run build       # all packages build
npm test            # all tests pass
npm run repopack   # generates v5 exports under ai/exports/
```

### Design decisions

- **Exports git-ignored** — the repo pack files are large and change every run; they don't belong in version control.
- **Version state tracked** — `ai/repomix-state.json` is committed so the counter stays consistent across sessions.
